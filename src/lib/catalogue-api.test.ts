...
import { listPublicProducts } from "./product-repository";

vi.mock("@/lib/product-repository", () => ({ listPublicProducts: vi.fn() }));

afterEach(() => vi.unstubAllGlobals());

describe("dynamic catalogue", () => {
  it("serves a cacheable public MongoDB catalogue", async () => {
    vi.mocked(listPublicProducts).mockResolvedValue(products);
    const response = await GET();
    expect(response.headers.get("Cache-Control")).toContain("s-maxage=60");
    expect(await response.json()).toEqual({ products, source: "database" });
  });
  it("uses fetched products and searches newly returned categories", async () => {
    ...
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(Response.json({ products: [product] })),
    );
    const result = await loadCatalogue(new AbortController().signal);
    expect(result.fallback).toBe(false);
    expect(
      filterProducts("New model", "New category", result.products),
    ).toHaveLength(1);
  });
  it("preserves a valid empty catalogue instead of inventing availability", async () => {
    ...
    expect(await loadCatalogue(new AbortController().signal)).toEqual({
      products: [],
      fallback: false,
    });
  });
    ...
    async (failure) => {
      ...
      if (failure === "network" || failure === "timeout")
        ...
        fetch.mockResolvedValue(
          Response.json(failure === "http" ? {} : { products: [null] }, {
  ...
  it("rejects duplicate IDs, oversized catalogues and unsafe URLs", () => {
    expect(() =>
      parseCatalogue({ products: [products[0], products[0]] }),
    ).toThrow();
    expect(() =>
      parseCatalogue({ products: Array(101).fill(products[0]) }),
    ).toThrow();
    expect(() =>
      parseCatalogue({
        products: [{ ...products[0], imageUrl: "javascript:alert(1)" }],
      }),
    ).toThrow();
  });
