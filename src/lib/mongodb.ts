...
declare global {
  var tirupatiMongo: Promise<MongoClient> | undefined;
}

function configuration() {
  const uri = process.env.MONGODB_URI?.trim() || "";
  const database = process.env.MONGODB_DATABASE?.trim() || "tirupati_coolers";
...
export async function database() {
  ...
  const client = global.tirupatiMongo ?? connect();
  if (process.env.NODE_ENV !== "production") global.tirupatiMongo = client;
