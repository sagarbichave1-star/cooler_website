...
export function CoolerVisual({ tone = "ocean", compact = false }: CoolerVisualProps) {
  return (
    <div className={`cooler-visual tone-${tone}${compact ? " is-compact" : ""}`} aria-hidden="true">
      <svg viewBox="0 0 520 580" role="img">
        <defs>
          <linearGradient id={`body-${tone}`} x1="0" y1="0" x2="1" y2="1">
            <stop offset="0" stopColor="var(--cooler-highlight)" />
            <stop offset="1" stopColor="var(--cooler-body)" />
          </linearGradient>
          <radialGradient id={`fan-${tone}`}>
            <stop offset="0" stopColor="#f9feff" />
            <stop offset="1" stopColor="var(--cooler-fan)" />
          </radialGradient>
          <filter id={`shadow-${tone}`} x="-30%" y="-30%" width="160%" height="180%">
        ...
        <g filter={`url(#shadow-${tone})`}>
          <path d="M141 71c0-20 16-36 36-36h166c20 0 36 16 36 36l24 406c1 22-16 40-38 40H155c-22 0-39-18-38-40l24-406Z" fill={`url(#body-${tone})`} />
          ...
          <circle cx="260" cy="270" r="103" fill={`url(#fan-${tone})`} stroke="#ffffff" strokeOpacity=".6" strokeWidth="5" />
